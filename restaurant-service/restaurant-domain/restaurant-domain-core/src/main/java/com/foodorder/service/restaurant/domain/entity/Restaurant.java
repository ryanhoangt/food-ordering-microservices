    ...
    public void validateOrder(List<String> failureMessages) {
        ...
        Money totalAmount = orderDetail.getProducts().stream().map(product -> {
            if (!product.isAvailable()) {
                failureMessages.add("Product with id: " + product.getId().getIdValue() +
                        " is not available");
            }
        ...
        if (!totalAmount.equals(orderDetail.getTotalAmount())) {
            failureMessages.add("Price total is not correct for order: " + orderDetail.getId().getIdValue());
        }
    }

    public void constructOrderApproval(RestaurantValidationStatus approvalStatus) {
